...
# - Seg-sex 11:57 (1-5). O envio ao n8n acontece em cadeia: fuzzing.js -> sync-api -> webhook.
# - flock -n: se a execucao anterior ainda roda, o cron pula (evita sobreposicao).
RUN printf '%s\n' \
    'PATH=/usr/local/bin:/usr/bin:/bin' \
    'TZ=America/Sao_Paulo' \
    'CHROME_PATH=/usr/bin/chromium' \
    '57 11 * * 1-5 flock -n /tmp/fuzzing.lock -c "cd /app && node fuzzing.js >> /var/log/cron-fuzzing.log 2>&1"' \
    > /app/crontab.txt \
