...
function sendJson(url, payload, extraHeaders = {}) {
...
async function postToWebhook(url, payload, extraHeaders = {}) {
  ...
  let first;
  try {
    first = await sendJson(url, payload, extraHeaders);
  } catch (err) {
  ...
  if (shouldRetry && testUrl !== url) {
    ...
    let second;
    try {
      second = await sendJson(testUrl, payload, extraHeaders);
    } catch (err) {
...
async function dispatchToWebhooks(payload) {
  ...
  // Assina a rota POST do Bubble com o JWT (HS256) ja existente, via header
  // Authorization: Bearer <token>. So o destino Bubble recebe o header; o n8n
  // segue sem headers extras (comportamento inalterado).
  const bubbleHeaders = jwtToken ? { Authorization: `Bearer ${jwtToken}` } : {};

  const targets = [
    { name: 'n8n', url: N8N_WEBHOOK, body: payload, headers: {} },
    { name: 'bubble', url: BUBBLE_WEBHOOK, body: bubbleBody, headers: bubbleHeaders },
  ];
  const settled = await Promise.allSettled(
    targets.map(t => postToWebhook(t.url, t.body, t.headers))
  );
