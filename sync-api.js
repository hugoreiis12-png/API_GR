...
const N8N_WEBHOOK = process.env.N8N_WEBHOOK_URL || 'http://192.168.0.231:5678/webhook-test/af-dump-trigger';
...
// Guarda contra disparos duplicados: o fs.watch (Windows emite 'change' varias
// vezes por gravacao) + a chamada explicita do fuzzing.js poderiam enviar o mesmo
// dump 2-3x, gerando 2-3 execucoes do workflow (e 2-3 planilhas/emails).
// Aqui garantimos no maximo 1 envio dentro da janela de cooldown.
let dispatchInFlight = false;
let lastDispatchAt = 0;
const DISPATCH_COOLDOWN_MS = Number(process.env.DISPATCH_COOLDOWN_MS || 8000);

async function dispatchOnce(filePath, source) {
  const now = Date.now();
  if (dispatchInFlight) {
    return { skipped: true, reason: 'envio em andamento', source };
  }
  if (now - lastDispatchAt < DISPATCH_COOLDOWN_MS) {
    return { skipped: true, reason: `cooldown ${DISPATCH_COOLDOWN_MS}ms`, source };
  }
  dispatchInFlight = true;
  try {
    const result = await dispatchFileToN8N(filePath, source);
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/dump-ready') {
    ...
    req.on('end', async () => {
      try {
        ...
        const result = await dispatchOnce(filePath, parsed.source || 'fuzzing.js');
        respondJson(res, 200, { ok: true, message: result.skipped ? 'Disparo duplicado ignorado' : 'Arquivo enviado ao n8n', result });
      } catch (error) {
  ...
  if (req.method === 'POST' && req.url === '/trigger') {
    try {
      const result = await dispatchOnce(FILE_PATH, 'manual-trigger');
      respondJson(res, 200, { ok: true, message: result.skipped ? 'Disparo duplicado ignorado' : 'Workflow disparado via webhook', result });
    } catch (error) {
      respondJson(res, 500, { ok: false, message: error.message });
    }
    return;
...
server.listen(PORT, () => {
  ...
  // Debounce do watch: o fs.watch dispara varios eventos por gravacao (sobretudo
  // no Windows). Reiniciamos um unico timer a cada evento e so enviamos uma vez.
  let watchTimer = null;
  fs.watch(__dirname, { persistent: true }, (eventType, filename) => {
    ...
    watchTimer = setTimeout(async () => {
      ...
      try {
        ...
        const result = await dispatchOnce(FILE_PATH, 'file-watch');
        if (result.skipped) {
          console.log('watch: disparo ignorado (', result.reason, ')');
        } else {
